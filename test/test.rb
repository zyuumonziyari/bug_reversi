...
class TestReversi < Minitest::Test
  ...
  def test_finished?
    refute finished?(build_board('pattern00.txt'))  # 初期盤面
    assert finished?(build_board('pattern03a.txt')) # 全て埋まった
    assert finished?(build_board('pattern03b.txt')) # 白最短勝利
    assert finished?(build_board('pattern03c.txt')) # 黒最短勝利
    refute finished?(build_board('pattern03d.txt')) # 白配置可・黒配置不可
    refute finished?(build_board('pattern03e.txt')) # 白配置不可・黒配置可
  end
end
